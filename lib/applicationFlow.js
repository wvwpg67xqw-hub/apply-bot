...
async function runApplication(client, user, sourceGuild, roleType) {
  ...
  // ── Online AI-text detection scan ─────────────────────────────────────────
  // Runs the applicant's combined answers through a real hosted AI-text
  // detector (Hugging Face's chatgpt-detector-roberta model) and posts a
  // verdict — human vs AI-generated — alongside the typing analysis above.
  let aiFlagged = false;
  try {
    ...
    if (combinedText.trim().length >= 20) {
      ...
      const aiEmbed = new EmbedBuilder()
        ...
        .setColor(aiColor)
        .setDescription("Scanned all answers using an online AI-text detector (Hugging Face).")
        .addFields(
