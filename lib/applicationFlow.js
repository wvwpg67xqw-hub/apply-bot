...
async function runApplication(client, user, sourceGuild, roleType) {
  ...
  if (await isBlacklisted(sourceGuild.id, user.id)) return { ok: false, reason: "blacklisted" };
  ...
  try {
    ...
    const introEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: `${questionSet.length} question${questionSet.length === 1 ? "" : "s"} total` });
  ...
  // ── Local AI-text heuristic scan ──────────────────────────────────────────
  // Runs entirely locally (no external API call, so it can't fail to
  // respond like the old Hugging Face integration did) and scores the
  // applicant's combined answers for common LLM-writing patterns.
  let aiFlagged = false;
  try {
    const combinedText = answers.join("\n\n");
    if (combinedText.trim().length >= 20) {
      const aiResult = analyzeText(combinedText);
      aiFlagged = aiResult.isAI;
      ...
      const aiEmbed = new EmbedBuilder()
        .setTitle(`${aiIcon} AI Detection Result`)
        .setColor(aiColor)
        .setDescription("Local heuristic scan for common AI-writing patterns (stock phrasing, uniform sentence length, lack of contractions/casual language).")
        .addFields(
          { name: "Verdict", value: aiVerdict,                    inline: false },
          { name: "Signals", value: aiResult.reasons.join("\n"), inline: false },
        )
        .setFooter({ text: `${aiResult.wordCount} words • ${aiResult.sentenceCount} sentences • heuristic score, not a certainty` })
        .setTimestamp();
      await thread.send({ embeds: [aiEmbed] });
    }
  } catch (err) {
    log.warn("DETECTAI", "AI heuristic scan failed for application", err.message);
    await thread.send({ content: `⚠️ AI detection scan failed: ${err.message}` }).catch(() => {});
  }

  const flaggedAny = flagged || aiFlagged;

  // Notify the parent channel without pinging — ping stays in the thread
  try {
    await appChannel.send({
      content: `📥 New **${meta.label}** application from **${user.tag}** (${sourceGuild.name}) — ID: \`${appId}\`${flaggedAny ? " 🚩 *(flagged: possible pasted/AI answer)*" : ""}\n> Review it in ${thread}`,
    });
  } catch (err) {
  ...
  log.info("APP", `Submitted | [${sourceGuild.name}] ${meta.label} → #${appChannel.name} in [${destGuild.name}] | ${user.tag} | ID: ${appId}${flaggedAny ? " | FLAGGED" : ""}`);
  return { ok: true };
}
