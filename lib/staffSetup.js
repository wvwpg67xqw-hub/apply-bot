...
async function resolveAppChannel(client, sourceGuild, guildConfig) {
  // 1. Explicit route saved by /setstaffserver
  ...
  // 2. Manually configured applicationChannel
  ...
  if (entry) {
    // 3. Search source guild itself
    for (const name of entry.channelNames) {
      const ch = sourceGuild.channels.cache.find((c) => c.name === name && c.isTextBased());
      if (ch) return { channel: ch, guild: sourceGuild };
    }

    // 4. Search the staff server as fallback
    const cfg = await getConfig();
    if (cfg.staffGuildId) {
      const staffGuild = client.guilds.cache.get(cfg.staffGuildId);
      if (staffGuild) {
        for (const name of entry.channelNames) {
          const ch = staffGuild.channels.cache.find((c) => c.name === name && c.isTextBased());
          if (ch) return { channel: ch, guild: staffGuild };
        }
      }
    }
  }
